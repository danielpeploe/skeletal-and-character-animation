...
void drawFloor() {
    ...
    for (int i = -1000; i <= 1000; i += 10) {
        for (int j = -1000; j <= 1000; j += 10) {
            if (light_green) glColor3f(0.47, 0.78, 0.48);
            else glColor3f(0.18, 0.60, 0.21);
            glVertex3f(i, 0, j);
            glVertex3f(i, 0, j + 10);
...
void transformVertices() 
{
    ...
    for (int imesh = 0; imesh < scene->mNumMeshes; imesh++)
    {
        ...
        for (int i = 0; i < mesh->mNumBones; i++) 
        {
            ...
            matProd = bone->mOffsetMatrix;
            
            while(node != NULL) 
            {
                matProd = node->mTransformation * matProd;
                node = node->mParent;
            }
            
            norMat = aiMatrix4x4(matProd).Inverse().Transpose();
            ...
            for(int k = 0; k < bone->mNumWeights; k++)
            {
                ...
                mesh->mVertices[vid] = matProd * vert;
                mesh->mNormals[vid] = norMat * norm;
            }
        }
    }
}
